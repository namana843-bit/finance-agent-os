...
function defaultStreamingSpawnImpl(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stderr.on("data", (chunk: Buffer) => {
      fullStderr += chunk.toString("utf-8");
    });
    ...
    child.on("close", (code: number) => {
      ...
      if (lineBuffer.trim()) {
        ...
        if (!handled) {
          onChunk({ type: "delta", content: lineBuffer });
        }
      }
...
export class CliProvider implements LLMProvider {
  ...
  private async resolvePath(cmd: string): Promise<string | null> {
    // Instance-level cache: autoResolveCliPath already caches globally,
    // this avoids even the overrides file read on hot chat path.
    if (this.resolvedCache !== undefined && Date.now() - this.resolvedAt < CliProvider.RESOLVE_TTL_MS) {
    ...
    try {
      ...
      const idStr = this.id;
      const baseId = idStr.startsWith("cli-") ? idStr.slice(4) : idStr;

      if (typeof overrides[baseId] === "string" && overrides[baseId].trim() !== "") {
        overriddenCmd = overrides[baseId].trim();
      } else if (typeof overrides[idStr] === "string" && overrides[idStr].trim() !== "") {
        overriddenCmd = overrides[idStr].trim();
      }
  ...
  /** Build a single CLI prompt from full history + tool defs.
   * Single user message without tools stays verbatim (backward compat).
   * Multi-turn/tool flows are flattened so the CLI sees system context and
   * can answer or emit {"tool_call":...} in ONE spawn instead of 3-5 loops. */
  private buildPrompt(
      ...
      if (m.role === "system") parts.push(`System: ${m.content}`);
        ...
        if (m.toolCalls?.length) {
          for (const tc of m.toolCalls) {
            parts.push(`Assistant tool call ${tc.name}: ${JSON.stringify(tc.arguments)}`);
          }
  ...
  async healthCheck(): Promise<ProviderHealth> {
    ...
    if (resolved) {
      return {
        providerId: this.id,
        status: "ok",
        message: `Executable resolved: ${resolved}`,
        latencyMs,
        modelsCount: 1,
      };
    }
    return {
      providerId: this.id,
      status: "unreachable",
      message: `CLI executable '${this.command}' not found on system PATH`,
      latencyMs,
    };
  }
    ...
    return [
      {
        ...
        provider: this.id,
        supportsTools: true,
        description: `Local executable: ${resolved}`,
      },
    ];
  }
  ...
  async *chat(
    ...
    if (!resolved) {
      yield {
        type: "error",
        messageId,
        error: `CLI executable '${this.command}' not found. Please verify executable path or installation.`,
        timestamp: Date.now(),
      };
      return;
    }
    ...
    try {
      ...
      yield {
        type: "error",
        messageId,
        error: `Failed to create CLI session: ${err instanceof Error ? err.message : String(err)}`,
        timestamp: Date.now(),
      };
      return;
    }
    ...
    try {
      for await (const chunk of session.sendPrompt(prompt, signal)) {
        if (chunk.type === "tool_call" && chunk.toolCall) {
          hasToolCall = true;
          yield {
            type: "tool_call",
            messageId,
            toolCallId: `call_cli_${Date.now()}`,
    ...
    if (!hasToolCall && completeText.trim().length === 0) {
      yield {
        type: "error",
        messageId,
        error: "CLI engine returned empty response",
        timestamp: Date.now(),
      };
      return;
    }
